...
public class ExecutionEnvUtil {
    public static ParameterTool createParameterTool(final String[] args) throws Exception {
        return ParameterTool
                .fromPropertiesFile(ExecutionEnvUtil.class.getResourceAsStream(Constants.PROPERTIES_FILE_NAME))
    ...
    public static StreamExecutionEnvironment prepare(ParameterTool parameterTool) throws Exception {
        ...
        if (parameterTool.getBoolean(Constants.STREAM_CHECKPOINT_ENABLE, true)) {
            env.enableCheckpointing(parameterTool.getInt(Constants.STREAM_CHECKPOINT_INTERVAL, 1000)); // create a checkpoint every 5 seconds
        }
